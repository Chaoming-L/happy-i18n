    ...
    return {
        visitor: {
            ConditionalExpression(path) {
                const {node} = path;
                detectChinese(node.consequent.value, path, 'text', 'ConditionalExpression');
                detectChinese(node.alternate.value, path, 'text', 'ConditionalExpression');
            },
            ReturnStatement(path) {
                const {node} = path;
                detectChinese(node.argument && node.argument.value, path, 'text', 'ReturnStatement');
            },
            JSXAttribute(path) {
                const { node } = path;
